...
export const searchItems = async (searchText: string) => {
    ...
    const items = results.map(
        ...
        return { id, title, price, picture: thumbnail, condition, free_shipping };
    });
...
export const getItemDetails = async (itemId: string) => {
    ...
    const { id, title, currency_id, price, pictures, attributes, shipping: {free_shipping}, sold_quantity  } = detailsResponse.data;
